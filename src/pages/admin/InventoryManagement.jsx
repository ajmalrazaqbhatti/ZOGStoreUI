...
function InventoryManagement() {
    ...
    const handleStockChange = (gameId, operation) => {
        ...
        const currentStock = stockChanges[gameId] !== undefined ? stockChanges[gameId] : inventoryItem.stock_quantity;
        ...
        <div className="min-h-screen bg-[#0A0A0B] text-white"
            ...
            <div className="lg:ml-64 transition-all duration-300">
                <div className="p-4 md:p-8">
                    {/* Toast Notification */}
                    {toast.visible && (
                        <div className={`fixed bottom-4 right-4 z-50 p-4 rounded-3xl shadow-2xl flex items-center gap-3 animate-fadeIn
                            bg-black/80 backdrop-blur-md border max-w-md 
                            ${toast.type === 'success' ? 'border-green-500/30' :
                                toast.type === 'updating' ? 'border-[#7C5DF9]/30' : 'border-red-500/30'}`}>
                            {toast.type === 'success' ? (
                                    ...
                                    <table className="min-w-full divide-y divide-white/10">
                                        ...
                                        <tbody className="divide-y divide-white/10">
                                            {filteredInventory.map(item => (
                                                <tr key={item.inventory_id} className={`hover:bg-white/5 ${updatingItem === item.game_id ? 'bg-[#7C5DF9]/5' : ''}`}>
